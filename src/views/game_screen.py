...
from views.game_screen_ideahint import IdeaHintMixin, get_idea_hint_batch
from views.game_screen_overlays import OverlayMixin
...
class GameScreen(WordFindMixin, BoardRulesMixin, BoardSetupMixin, SelectionMixin,
    ...
    def __init__(self, window, screen_manager):
        ...
        # The right-click-for-an-idea cell hint (game_screen.idea_hint_*), which
        # shares only the generated word/emoji file with the belt above -- see
        # views/game_screen_ideahint. Resolved here so its rules and empty state
        # exist before the first click, whether or not the belt is on.
        self._setup_idea_hint()
    ...
    # Extracted to OverlayMixin (views/game_screen_overlays.py); see there.
    ...
    def draw(self):
        ...
        # Idea-hint pictures, drawn AFTER the three cell batches. They read as a
        # cell background, but they cannot be drawn before those batches: a cell's
        # fill is an OPAQUE rectangle living in the same batch as its letters, so
        # anything drawn earlier is painted straight over. Translucency does the
        # work instead -- the picture sits on the cell like a watermark and the
        # gram stays legible through it (game_screen.idea_hint_opacity). Empty
        # unless a player has double-clicked a multigram cell.
        get_idea_hint_batch().draw()
    ...
    def update(self, dt):
        ...
        # Idea hints follow their cell: a picture whose gram has been cleared off
        # the board (or changed under it) comes down here. A dict-empty test in
